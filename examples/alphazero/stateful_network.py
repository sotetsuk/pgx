...
class BlockV1(eqx.Module):
    ...
    def __init__(self, in_channels, out_channels, key):
        ...
        self.conv1 = eqx.nn.Conv2d(in_channels, out_channels, kernel_size=3, key=keys[0])
        self.conv2 = eqx.nn.Conv2d(out_channels, out_channels, kernel_size=3, key=keys[1])
...
class AZNet(eqx.Module):
    ...
    def __init__(
        ...
        self.policy_head = [
            ...
            eqx.nn.BatchNorm(output_channels, "batch", momentum=0.9),
            jax.nn.relu,
            lambda x: x.flatten(),
            eqx.nn.Linear(200, self.num_actions, key=keys[num_blocks + 2]),
        ]
        ...
        self.value_head = [
            ...
            jax.nn.relu,
            lambda x: x.flatten(),
            eqx.nn.Linear(200, output_channels, key=keys[num_blocks + 2]),
            jax.nn.relu,
