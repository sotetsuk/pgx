...
class State(core.State):
    ...
    def env_id(self) -> core.EnvId:
        try:
            size = int(self._game_state._size.item())
        except TypeError:
            size = int(self._game_state._size[0].item())
...
def _observe(state: State, player_id, size, history_length):
    ...
    my_turn = jax.lax.select(
        player_id == state.current_player, state._game_state._turn, 1 - state._game_state._turn
    )
    ...
    def _make(i):
        ...
        return state._game_state._board_history[i // 2] == color
...
def _step(state: State, action: int, size: int) -> State:
    state = state.replace(
        _game_state=state._game_state.replace(
            _ko=jnp.int32(-1)
        )  # type: ignore
    )
    # update state
    ...
    state = state.replace(
        _game_state=state._game_state.replace(
            _turn=(state._game_state._turn + 1) % 2
        )
    )  # type: ignore
    ...
    board_history = jnp.roll(state._game_state._board_history, size**2)
    board_history = board_history.at[0].set(
        jnp.clip(state._game_state._chain_id_board, -1, 1).astype(jnp.int32)
    )
    state = state.replace(
        _game_state=state._game_state.replace(
...
def _not_pass_move(state: State, action, size) -> State:
    ...
    xy = action
    num_captured_stones_before = state._game_state._num_captured_stones[state._game_state._turn]
    ...
    chain_id = state._game_state._chain_id_board[adj_xy]
    ...
    # fmt: off
    state = jax.lax.cond(
        state._game_state._num_captured_stones[state._game_state._turn] - num_captured_stones_before == 1,
        lambda: state,
        lambda: state.replace(
            _game_state=state._game_state.replace(
                _ko=jnp.int32(-1)),
...
def _merge_around_xy(i, state: State, xy, size):
    ...
    is_off = adj_xy == -1
    is_my_chain = state._game_state._chain_id_board[adj_xy] * my_color > 0
    state = jax.lax.cond(
        ((~is_off) & is_my_chain),
...
def _set_stone(state: State, xy) -> State:
    ...
    return state.replace(  # type:ignore
        _game_state=state._game_state.replace(
            _chain_id_board=state._game_state._chain_id_board.at[xy].set((xy + 1) * my_color),
        )
    )
...
def _merge_chain(state: State, xy, adj_xy):
    my_color = _my_color(state)
    new_id = jnp.abs(state._game_state._chain_id_board[xy])
    adj_chain_id = jnp.abs(state._game_state._chain_id_board[adj_xy])
    ...
    chain_id_board = jnp.where(
        state._game_state._chain_id_board == large_id, small_id, state._game_state._chain_id_board
    )
...
def _remove_stones(
    ...
    surrounded_stones = state._game_state._chain_id_board == rm_chain_id
    num_captured_stones = jnp.count_nonzero(surrounded_stones)
    chain_id_board = jnp.where(surrounded_stones, 0, state._game_state._chain_id_board)
    ko = jax.lax.cond(
        ko_may_occur & (num_captured_stones == 1),
        lambda: jnp.int32(rm_stone_xy),
        lambda: state._game_state._ko,
    )
    return state.replace(  # type:ignore
        _game_state=state._game_state.replace(
            _chain_id_board=chain_id_board,
            _num_captured_stones=state._game_state._num_captured_stones.at[state._game_state._turn].add(
                num_captured_stones
            ),
            _ko=ko,
        )
...
def legal_actions(state: State, size: int) -> Array:
    ...
    is_empty = state._game_state._chain_id_board == 0

    my_color = _my_color(state)
    opp_color = _opponent_color(state)
    num_pseudo, idx_sum, idx_squared_sum = _count(state, size)

    chain_ix = jnp.abs(state._game_state._chain_id_board) - 1
    # fmt: off
    in_atari = (idx_sum[chain_ix] ** 2) == idx_squared_sum[chain_ix] * num_pseudo[chain_ix]
    # fmt: on
    has_liberty = (state._game_state._chain_id_board * my_color > 0) & ~in_atari
    kills_opp = (state._game_state._chain_id_board * opp_color > 0) & in_atari
    ...
    return jax.lax.cond(
        (state._game_state._ko == -1),
        lambda: legal_action_mask,
        lambda: legal_action_mask.at[state._game_state._ko].set(FALSE),
    )


def _count(state: State, size):
    ZERO = jnp.int32(0)
    chain_id_board = jnp.abs(state._game_state._chain_id_board)
...
def _my_color(state: State):
    return jnp.int32([1, -1])[state._game_state._turn]
...
def _ko_may_occur(state: State, xy: int) -> Array:
    size = state._game_state._size
    x = xy // size
    y = xy % size
    oob = jnp.bool_([x - 1 < 0, x + 1 >= size, y - 1 < 0, y + 1 >= size])
    oppo_color = _opponent_color(state)
    is_occupied_by_opp = (
        state._game_state._chain_id_board[_neighbour(xy, size)] * oppo_color > 0
    )
...
def _count_point(state, size):
    return jnp.array(
        [
            ...
            + jnp.count_nonzero(state._game_state._chain_id_board > 0),
            _count_ji(state, -1, size)
            + jnp.count_nonzero(state._game_state._chain_id_board < 0),
        ],
        dtype=jnp.float32,
...
def _get_reward(state: State, size: int) -> Array:
    ...
    reward_bw = jax.lax.cond(
        score[0] - state._game_state._komi > score[1],
        lambda: jnp.array([1, -1], dtype=jnp.float32),
        lambda: jnp.array([-1, 1], dtype=jnp.float32),
    )
    reward = jax.lax.cond(
        state._game_state._black_player == 0,
        lambda: reward_bw,
...
def _count_ji(state: State, color: int, size: int):
    board = jnp.zeros_like(state._game_state._chain_id_board)
    board = jnp.where(state._game_state._chain_id_board * color > 0, 1, board)
    board = jnp.where(state._game_state._chain_id_board * color < 0, -1, board)
...
def _check_PSK(state):
    ...
    """
    # fmt: off
    is_psk = ~state._game_state._passed & (jnp.abs(state._game_state._board_history[0] - state._game_state._board_history[1:]).sum(axis=1) == 0).any()
...
def _show(state: State) -> None:
    ...
    POINT_CHAR = "+"
    print("===========")
    for xy in range(state._game_state._size * state._game_state._size):
        if state._game_state._chain_id_board[xy] > 0:
            print(" " + BLACK_CHAR, end="")
        elif state._game_state._chain_id_board[xy] < 0:
            print(" " + WHITE_CHAR, end="")
        else:
            print(" " + POINT_CHAR, end="")

        if xy % state._game_state._size == state._game_state._size - 1:
            print()
