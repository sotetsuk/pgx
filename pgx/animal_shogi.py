...
def move(board, turn, fir_lo, fin_lo, piece, captured, is_promote):
    turn_change(board, turn)
    board[fir_lo] = DEFAULT
    board[fin_lo] = np.roll(DEFAULT, piece+5*turn+4*is_promote)
    if captured == 0:
        return
    board[11+captured % 4+3*turn] = np.roll(board[11+captured % 4+3*turn], 1)
    return


def drop(board, turn, point, piece):
    turn_change(board, turn)
    board[11+piece+3*turn] = np.roll(board[11+piece+3*turn], -1)
    board[point] = np.roll(board[point], piece + 5*turn)
    return
