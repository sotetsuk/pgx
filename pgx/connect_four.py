...
class State(core.State):
    ...
    observationArray = jnp.zeros((6, 7, 2), dtype=jnp.bool_)
    rewardsArray = jnp.float32([0.0, 0.0])
    terminatedArray = FALSE
    truncatedArray = FALSE
    legal_action_maskArray = jnp.ones(7, dtype=jnp.bool_)
    ...
    _boardArray = -jnp.ones(42, jnp.int32)  # -1 (empty), 0, 1
    _blank_rowArray = jnp.full(7, 5)
