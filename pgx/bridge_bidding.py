...
class State(core.State):
    current_playerArray = jnp.int32(-1)
    observationArray = jnp.zeros(478, dtype=jnp.bool_)
    rewardsArray = jnp.float32([0, 0, 0, 0])
    terminatedArray = FALSE
    truncatedArray = FALSE
    legal_action_maskArray = jnp.ones(38, dtype=jnp.bool_)
    _step_countArray = jnp.int32(0)
    _turnArray = jnp.int32(0)
    _shuffled_playersArray = jnp.zeros(4, dtype=jnp.int32)
    # Hand of each player
    #   0  ~ 12: Hand of N
    #   13 ~ 25: Hand of E
    #   26 ~ 38: Hand of S
    ...
    _handArray = jnp.zeros(52, dtype=jnp.int32)
    ...
    _bidding_historyArray = jnp.full(319, -1, dtype=jnp.int32)
    # dealer: a player who starts bidding
    # 0 = N, 1 = E, 2 = S, 3 = W
    _dealerArray = jnp.zeros(4, dtype=jnp.int32)
    # vul_NS: Is NS team vul?
    # 0 = non vul, 1 = vul
    _vul_NSArray = jnp.bool_(False)
    # vul_EW: Is EW team vul?
    # 0 = non vul, 1 = vul
    _vul_EWArray = jnp.bool_(False)
    # last_bid
    # last_bidder
    # call_x: Was the last bid doubled?
    # call_xx: Was the last bid redoubled?
    _last_bidArray = jnp.int32(-1)
    _last_bidderArray = jnp.int32(-1)
    _call_xArray = jnp.bool_(False)
    _call_xxArray = jnp.bool_(False)
    # In NS team, which player first bid each denomination
    # Denomination order: C, D, H, S, NT = 0, 1, 2, 3, 4
    _first_denomination_NSArray = jnp.full(5, -1, dtype=jnp.int32)
    # In EW team, which player first bid each denomination
    _first_denomination_EWArray = jnp.full(5, -1, dtype=jnp.int32)
    # Number of pass
    _pass_numArray = jnp.array(0, dtype=jnp.int32)
...
def _init_by_key(keyArray, rngArray) -> State:
...
def _shuffle_players(rngArray) -> jax.Array:
    """Randomly arranges player IDs in a list in NESW order.

    Returns:
        jax.Array : A list of 4 player IDs randomly arranged in NESW order.
...
def _player_position(playerArray, state: State) -> jax.Array:
    return jax.lax.cond(
        player != -1,
...
def _convert_card_pgx_to_openspiel(cardArray) -> jax.Array:
...
def _position_to_team(positionArray) -> jax.Array:
...
def _is_partner(position1Array, position2Array) -> jax.Array:
...
def _state_to_key(state: State) -> jax.Array:
...
def _pbn_to_key(pbn: str) -> jax.Array:
...
def _to_binary(xArray) -> jax.Array:
...
def _key_to_hand(keyArray) -> jax.Array:
    """Convert key to hand"""
...
def _value_to_dds_tricks(valueArray) -> jax.Array:
...
def _find_value_from_key(
    keyArray, lut_keysArray, lut_valuesArray
):
...
def _load_sample_hash() -> Tuple[jax.Array, jax.Array]:
    # fmt: off
