...
class BridgeBidding(core.Env):
    def __init__(self):
        ...
        # fmt: ons
        with open(
            "/Users/kitayuu/workspace/pgx/workspace/keys_bytes.txt", "rb"
        ) as f:
            keys_byte = f.read()
            self.hash_keys = from_bytes(jnp.zeros((50000, 4)), keys_byte)
        with open(
            "/Users/kitayuu/workspace/pgx/workspace/values_bytes.txt", "rb"
        ) as f:
            values_byte = f.read()
            self.hash_values = from_bytes(jnp.zeros((50000, 4)), values_byte)
