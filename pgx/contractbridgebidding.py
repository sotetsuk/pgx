...
class ContractBridgeBiddingState:
    # turn 現在のターン数
    turn: np.ndarray = np.zeros(1, dtype=np.int32)
    # hand 各プレイヤーの手札
    # index = 0 ~ 12がN, 13 ~ 25がE, 26 ~ 38がS, 39 ~ 51がWの持つ手札
    # 各要素にはカードを表す0 ~ 51の整数が格納される
    hand: np.ndarray = np.zeros(52, dtype=np.int32)
    # bidding_history 各プレイヤーのbidを時系列順に記憶
    # 最大の行動系列長 = 525
    # 各要素には、bidを表す0 ~ 37の整数が格納される
    # 各ビッドがどのプレイヤーにより行われたかは、要素のindexから分かる
    bidding_history: np.ndarray = np.zeros(525, dtype=np.int32)
    # dealer どのプレイヤーがdealerかを表す
    # 0 = N, 1 = E, 2 = S, 3 = W
    # dealerは最初にbidを行うプレイヤー
    dealer: np.ndarray = np.zeros(4, dtype=np.int32)
    ...
    # が最初にbidしたかを表す
    first_denomination_NS: np.ndarray = np.zeros(5, dtype=np.int32)
    # first_denominaton_EW EWチームにおいて、各denominationをどのプレイヤー
    # が最初にbidしたかを表す
    first_denomination_EW: np.ndarray = np.zeros(5, dtype=np.int32)
