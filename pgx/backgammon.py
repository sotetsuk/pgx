...
class State(core.State):
    ...
    rng: jax.random.KeyArray = jnp.zeros(2, dtype=jnp.uint8)
    # サイコロの出目 0~5: 1~6
...
def _calc_src(src: int, turn: jnp.ndarray) -> int:
    ...
    boardのindexに合わせる.
    """
    return (src == 1) * jnp.int8(_bar_idx(turn)) + (src != 1) * jnp.int8(
        src - 2
    )  # type: ignore
...
def _from_other_than_bar(src: int, turn: jnp.ndarray, die: int) -> int:
    ...
    return _is_from_board * jnp.int8(src + die * -1 * turn) + (
        (~_is_from_board)
    ) * jnp.int8(
        _off_idx(turn)
    )  # type: ignore
...
def _legal_action_mask_for_single_die(
    ...
    """
    return (die == -1) * jnp.zeros(26 * 6 + 6, dtype=jnp.int8) + (
        die != -1
...
def _legal_action_mask_for_valid_single_dice(
    ...
    def _is_legal(idx: jnp.ndarray):
        ...
        legal_action_mask = jnp.zeros(26 * 6 + 6, dtype=jnp.int8)
